...
func SetWallpaper(input string) error {
	...
	conf := loadConfig()
	args := []string{"-s", filevalidator.QuotePath(abs)}
	if conf.Mode != "" {
		args = append(args, "-t", conf.Mode)
	}

	cmd := exec.Command("hyprpaper", args...)
	err = cmd.Run()
	if err != nil {
...
func loadConfig() config {
	...
	for _, entry := range entries {
		if !entry.IsDir() && strings.EqualFold(entry.Name(), "hyprpaper.toml") {
	...
	if configPath == "" {
		log.Println("No hyprpaper.toml configuration file found, using defaults")
		return config{}
	}

	data, err := os.ReadFile(configPath)
	if err != nil {
		log.Printf("Failed to read hyprpaper.toml: %v", err)
		return config{} // Return default if file can't be read
	}

	var config config
	if err := toml.Unmarshal(data, &config); err != nil {
		log.Printf("Failed to parse hyprpaper.toml: %v", err)
